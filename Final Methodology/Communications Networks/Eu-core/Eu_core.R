...
edges<-read.delim("email-Eu-core-department-labels.txt",header = TRUE, sep = " ")
...
write.csv(crsc, "crsc_Eu_core.csv")

dg<-decompose.graph(g)
temp<-sapply(dg,crossclique)
temp <- unlist(datalist)
big_data <- as.data.frame(temp)
aabb<-as.data.frame(as.numeric(row.names(big_data)))
aabb[,2]<-big_data

library(dplyr)
library(tidyr)
df<-arrange(aabb,`as.numeric(row.names(big_data))`)
...
#------------------------------------------Write data start-------------------------------------#
write.csv(centrality,"Centrality_Eu_core.csv")
write.csv(ncentrality,"NCentrality_Eu_core.csv")
...
projectionpoints=NeRV(as.matrix(var8), neighbors = 30, randominit = T, iterations = 5)
