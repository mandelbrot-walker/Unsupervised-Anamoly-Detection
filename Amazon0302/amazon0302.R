...
transitivity(g) #for checking computability of cross-clique centrality 
...
frm<-closeness.freeman(g) # Not calculatable as graph is not strongly connected
...
#------------------------------------------Write data start-------------------------------------#
write.csv(centrality,"Centrality_amazon0302.csv") #non normalized 8 centralities
write.csv(ncentrality,"NCentrality_amazon0302.csv") #normalized 6 centralities
write.csv(ncentrality2,"NCentrality_amazon0302.csv") #normalized 8 centralities
#------------------------------------------Write data end-------------------------------------#
...
res.pca22<-prcomp(ncen_tr[,-1],  scale = TRUE,center=TRUE) #extra pca to plot with ellipse !-- doesn't work well --!
...
fviz_pca_biplot(res.pca22, 
                ...
                gradient.cols = "RdBu"
)+  labs(fill = "Variables", color = "Contrib", alpha = "Contrib") # ellipse doesn't work 
...
## --------------------------------------------First attempt at tsne,kmeans,hierarchical------------------------------------#
set.seed(9)  
